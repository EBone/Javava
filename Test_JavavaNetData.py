...
def handle_data(classname,data):
    if isinstance(data,str) or isinstance(data,bytes):
        ...
        pack_result = pack_data(classname, send_id, recv_id,*data)
    print(pack_result)
    unpack_result = unpack_data(pack_result)
    print(unpack_result)
...
class TestJavavaMethods(unittest.TestCase):
    ...
    def test_Task_onlog(self):
        ...
        self.assertEqual(unpack_result[1][0],send_id)
        self.assertEqual(unpack_result[1][1], recv_id)
        self.assertEqual(unpack_result[2][0],content.encode('utf-8'))

    def test_Task_new(self):
        content =(b'a'*32,b'b'*32,0x11,0x22,0x33,0x44,'rtmp:123.123.123.123'.encode('utf-8'))
    ...
    def test_Task_gameresult_query(self):
        ...
        self.assertEqual(unpack_result[1][0], send_id)
        self.assertEqual(unpack_result[1][1], recv_id)
        self.assertEqual(unpack_result[2][0], content)
    ...
    def test_Task_game_command(self):
        for i in GameCommand.getcommand():
            content=(b"c"*32,0x1234,i)
    ...
    def test_Task_gamenew_apply(self):
        ...
        self.assertEqual(unpack_result[1][0], send_id)
        self.assertEqual(unpack_result[1][1], recv_id)
        self.assertEqual(tuple(unpack_result[2]), (content[0].encode('utf-8'),content[1].encode('utf-8')))
    def test_Task_Jvv_login(self):
        content=("abcd"*5,)
        pack_result, unpack_result = handle_data(MessageNames.Task_Jvv_Login, content)
        self.assertEqual(unpack_result[1][0], send_id)
        self.assertEqual(unpack_result[1][1], recv_id)
        self.assertEqual(tuple(unpack_result[2]), (content[0].encode('utf-8'),))
